...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("auto\\s+(?i)(white)", this::doAuto),
        new Command("manual\\s+(?i)(white)", this::doManual),
        new Command("([a-j][0-9][0-9]?)-([a-j]"
                +
                "[0-9][0-9]?)(\\([a-j][0-9][0-9]?\\))", this::doMove),
        new Command("([a-j]*[0-9]+\\s?)([a-j]"
                +
                "*[0-9]+\\s?)([a-j]*[0-9]+\\s?)", this::doMove),
        new Command("auto\\s+(?i)(black)", this::doAuto),
        new Command("manual\\s+(?i)(black)", this::doManual),
    ...
    /** Command to move.
     * @param matcher the move to match.*/
    private void doMove(Matcher matcher) {
        try {
            String from = matcher.group(1).trim();
            String to = matcher.group(2).trim();
            String spear = matcher.group(3).replace("(", "");
            spear = spear.replace(")", "");
            spear = spear.trim();
            Square fromSquare = Square.sq(from);
            Square toSquare = Square.sq(to);
            Square spearSquare = Square.sq(spear);
            Move mv = Move.mv(fromSquare, toSquare, spearSquare);
            if (board().isLegal(mv) && !(board().getMoves().contains(mv))) {
                board().makeMove(mv);
            }
            _winner = board().winner();
        } catch (IndexOutOfBoundsException | NullPointerException excp) {
            excp.printStackTrace();
            throw error("invalid move");
        }
    }

    /** Command auto, specifies which COLOR is going to be an AI. */
    private void doAuto(Matcher color) {
        String bW = color.group(1).trim().toLowerCase();
        if (bW.equals("white")) {
    ...
    /** Command manual, specifies which COLOR is going to be a human. */
